...
export default function LoginScreen({ serverIP, setServerIP, onBack, onLogin }) {
  ...
  const handleLegacyLogin = async () => {
    try {
      ...
      if (!response.ok) {
        setError(data.msg || 'Credenciais invalidas.');
      } else {
        onLogin({ accessToken: data.access_token, role: data.role, username: data.username || user, status: data.status });
      }
    } catch {
    ...
    if (isSignUp) {
      ...
      try {
        ...
        if (signUpError) {
          ...
          // If the user was created, show success message
          if (data?.user) {
            setSuccessMsg('Conta criada com sucesso! Aguardando aprovação do administrador.');
            setIsSignUp(false);
            setPass('');
            ...
            onLogin({
             ...
             setSuccessMsg('Verifique seu e-mail para confirmar a conta (caso aplicável).');
          }
        }
      } catch {
      ...
      // Login - Tenta primeiro com Supabase (se estiver configurado)
      if (supabase.supabaseUrl && user.includes('@')) {
          try {
              ...
              if (signInError) {
                  // Se falhar no Supabase e não for legacy, tenta legacy como fallback
          ...
          // Utilizador normal não-email (ex: admin local)
          await handleLegacyLogin();
          setLoading(false);
      }
    }
  ...
  const handleOAuthSignIn = async (provider) => {
    ...
    try {
      ...
      setError(`Erro ao iniciar sessão com ${provider}.`);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="w-full max-w-md bg-slate-900/80 p-6 sm:p-8 rounded-3xl border border-slate-800 relative">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          <div className="relative">
            {isSignUp ? <Mail className="absolute left-4 top-3.5 text-slate-500" size={20} /> : <User className="absolute left-4 top-3.5 text-slate-500" size={20} />}
            <input
              type={isSignUp ? "email" : "text"}
              placeholder={isSignUp ? "E-mail" : "Usuario ou E-mail"}
              value={user}
          ...
          {successMsg && <div className="bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 p-3 rounded-xl text-sm flex items-center justify-center gap-2 text-center"><span>{successMsg}</span></div>}

          <button disabled={loading} className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-4 rounded-xl flex justify-center items-center gap-2 disabled:opacity-50 transition-colors">
            {loading ? <Activity className="animate-spin" size={20} /> : (isSignUp ? <><Plus size={20} /> Criar Conta</> : <><LogIn size={20} /> Entrar</>)}
          </button>
        </form>
        ...
        <div className="mt-8 text-center text-sm text-slate-500">
          {isSignUp ? (
            <p>
              Já tem uma conta?{' '}
              <button onClick={() => { setIsSignUp(false); setError(''); setSuccessMsg(''); }} className="text-emerald-500 hover:text-emerald-400 font-medium transition-colors">
                Fazer login
              </button>
            </p>
          ) : (
            <p>
              Não tem uma conta?{' '}
              <button onClick={() => { setIsSignUp(true); setError(''); setSuccessMsg(''); }} className="text-emerald-500 hover:text-emerald-400 font-medium transition-colors">
                Criar agora
              </button>
            </p>
          )}
