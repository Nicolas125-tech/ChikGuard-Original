...
## 2025-08-11 - Static aria-label on dynamic loading buttons
**Learning:** Adding a static `aria-label` to a button that contains dynamic visible text (like changing from "Save" to "Saving..." during an async operation) is an accessibility anti-pattern. The static `aria-label` overrides the visible text for screen readers, hiding the crucial loading state feedback from visually impaired users.
**Action:** Only use `aria-label` on icon-only buttons. For buttons with descriptive visible text, let the native text serve as the label. If an `aria-label` must be used on a dynamic button, ensure the `aria-label` itself is also dynamic and reflects the loading state.
