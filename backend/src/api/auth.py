...
def _accounts_users_handler(deps):
...
def _accounts_user_update_handler(account_id, deps):
    guard_critical_action = deps.get("guard_critical_action")
...
def _accounts_user_delete_handler(account_id, deps):
    guard_critical_action = deps.get("guard_critical_action")
    audit = deps.get("audit")
...
def _accounts_permissions_handler(deps):
    guard_critical_action = deps.get("guard_critical_action")
    audit = deps.get("audit")
    db = deps.get("db")
...
def _admin_pending_users_handler(deps):
    guard_critical_action = deps.get("guard_critical_action")
...
def _admin_approve_user_handler(deps):
    guard_critical_action = deps.get("guard_critical_action")
    audit = deps.get("audit")
...
def _webhook_notify_new_user_handler():
