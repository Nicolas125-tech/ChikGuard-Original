...
def handle_auto_mode(deps):
    ...
    audit(
        ...
        source="manual",
        details={
            "enabled": estado_dispositivos["modo_automatico"],
            "config": auto_config,
        },
    )
    return jsonify(
        {"enabled": estado_dispositivos["modo_automatico"], "config": auto_config}
    )


def handle_controlar_ventilacao(deps):
    guard_critical_action = deps.get("guard_critical_action")
    log_event = deps.get("log_event")
    audit = deps.get("audit")
    estado_dispositivos = deps.get("estado_dispositivos")
...
def handle_controlar_aquecedor(deps):
    guard_critical_action = deps.get("guard_critical_action")
    log_event = deps.get("log_event")
    audit = deps.get("audit")
    estado_dispositivos = deps.get("estado_dispositivos")
...
def handle_controlar_luz_dimmer(deps):
    guard_critical_action = deps.get("guard_critical_action")
    log_event = deps.get("log_event")
    audit = deps.get("audit")
    estado_dispositivos = deps.get("estado_dispositivos")
    ...
    if request.method == "GET":
        return jsonify(
            {
                "luz_intensidade_pct": int(
                    estado_dispositivos.get("luz_intensidade_pct", 0)
                )
            }
        )

    ok, resp = guard_critical_action(
        "light_dimmer_change", permission="lighting.manage"
    )
    if not ok:
        return resp
    ...
    log_event(
        "light_dimmer_changed",
        "info",
        f"Intensidade da luz ajustada para {intensidade}%",
    )
    audit(
        "light_dimmer_changed",
        source="manual",
        details={"intensidade_pct": intensidade},
    )
    return jsonify({"luz_intensidade_pct": intensidade, "msg": "Dimmer atualizado"})


def handle_get_estado_dispositivos(deps):
    estado_dispositivos = deps.get("estado_dispositivos")
