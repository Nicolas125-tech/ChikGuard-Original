...
def handle_get_sensors_live(deps):
...
def handle_get_sensors_history(deps):
    active_camera_id = deps.get("active_camera_id")
...
def handle_ingest_sensor_data(deps):
    sensor_state = deps.get("sensor_state")
...
def handle_thermal_anomalies_live(deps):
    active_camera_id = deps.get("active_camera_id")
    ThermalAnomaly = deps.get("ThermalAnomaly")
    utcnow = deps.get("utcnow")
    timedelta_cls = deps.get("timedelta")
...
def handle_acoustic_history(deps):
    active_camera_id = deps.get("active_camera_id")
...
def handle_check_huddling(deps):
    utcnow = deps.get("utcnow")
    timedelta_cls = deps.get("timedelta")
    log_event = deps.get("log_event")
    ...
    start = utcnow() - timedelta_cls(minutes=3)
