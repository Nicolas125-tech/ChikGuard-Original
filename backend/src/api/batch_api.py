...
def create_batch_blueprint(deps):
    ...
    @require_auth()
    def get_batches():
        batches = Batch.query.filter_by(camera_id=_camera_id).order_by(Batch.start_date.desc()).all()
    ...
    def start_batch():
        ...
        if _audit: _audit("batch_started", details={"name": name})
    ...
    def close_batch():
        ...
        if _audit: _audit("batch_closed", details={"id": active.id})
