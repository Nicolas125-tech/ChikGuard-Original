...
class AutomationEngine:
    ...
    def process_telemetry(self, camera_id: str, temp_c: float, humidity_pct: float):
        ...
        if temp_c > 31.0:
            self._trigger_action(camera_id, "exhaust_fan", "on", reason=f"Temperatura critica ({temp_c}°C)")
        elif temp_c < 25.0:
            self._trigger_action(camera_id, "exhaust_fan", "off", reason=f"Temperatura normalizada ({temp_c}°C)")
            self._trigger_action(camera_id, "heater", "on", reason=f"Temperatura baixa detectada ({temp_c}°C)")
        elif temp_c > 28.0:
            self._trigger_action(camera_id, "heater", "off", reason=f"Temperatura adequada alcançada ({temp_c}°C)")
    ...
    def process_ai_vision_anomaly(self, camera_id: str, anomaly_type: str, severity: str):
        ...
        if anomaly_type == "thermal_crowding" and severity in ["high", "critical"]:
            ...
            self._trigger_action(camera_id, "heater", "on", reason="Aves amontoadas (IA detectou frio extremo)")
            ...
            self._trigger_action(camera_id, "exhaust_fan", "on", reason="Estresse térmico visual (IA detectou calor)")
            self._trigger_action(camera_id, "water_pump", "on", reason="Aumentar nebulização evaporativa")
    ...
    def process_ai_audio_anomaly(self, camera_id: str, cough_idx: float, stress_idx: float):
        ...
        """
        if cough_idx > 70.0:
            logger.warning(f"[Automação] IA Acústica detectou TOSSE ALTA ({cough_idx}%) em {camera_id}.")
            self._trigger_action(camera_id, "exhaust_fan", "on", reason=f"Renovação de ar de emergência (Amônia/Tosse: {cough_idx}%)")
    ...
    def _trigger_action(self, camera_id: str, device: str, state: str, reason: str):
        ...
        logger.info(f"⚡ [Automação] AÇÃO DISPARADA: {device.upper()} -> {state.upper()} ({reason})")
