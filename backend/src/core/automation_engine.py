    ...
    def process_telemetry(
        ...
        if ammonia_ppm > 20.0:
            self._trigger_action(
                ...
                "exhaust_fan",
                "on",
                reason=f"Renovação de ar por amônia elevada ({ammonia_ppm:.1f} ppm > 20.0 ppm)",
            )
        # Regras Dinâmicas
        ...
        if self.app_context_fn:
            ...
            # O(1) Local Cache implementation to avoid N+1 queries during batch processing
            if not hasattr(self, "_active_batch_cache"):
                self._active_batch_cache = {}
                self._active_batch_cache_time = 0

            # Cache active batches for 5 minutes (300 seconds)
            now = time.time()
            if now - self._active_batch_cache_time > 300:
                with self.app_context_fn():
                    try:
                        self._active_batch_cache.clear()
                        # Use an optimized single query to fetch all active batches for all cameras
                        # This prevents the N+1 query problem where we query the DB inside a loop
                        all_active_batches = Batch.query.filter_by(active=True).all()

                        # Store only the latest batch per camera
                        for b in all_active_batches:
                            # Only keep the latest one if there are multiple active for same camera
                            if (
                                b.camera_id not in self._active_batch_cache
                                or b.id > self._active_batch_cache[b.camera_id].id
                            ):
                                self._active_batch_cache[b.camera_id] = b

                        self._active_batch_cache_time = now
                    except Exception as db_err:
                        logger.error(
                            f"Erro ao pre-carregar lotes ativos para fallback de automacao: {db_err}"
                        )

            # Use cached batch
            batch = self._active_batch_cache.get(camera_id)
            if batch:
                try:
                    age_day = max(
                        1, (datetime.utcnow().date() - batch.start_date.date()).days + 1
                    )
                    target_temp = get_ideal_temp_for_age(age_day)
                except Exception as db_err:
                    logger.error(
                        f"Erro ao calcular target_temp para fallback de automacao: {db_err}"
                    )
        ...
        if temp_c > fan_on_temp:
            self._trigger_action(
                ...
                "exhaust_fan",
                "on",
                reason=f"Calor acima do conforto ({temp_c:.1f}°C > {fan_on_temp:.1f}°C)",
            )
            ...
            self._trigger_action(
                ...
                "exhaust_fan",
                "off",
                reason=f"Frio detectado. Ventilacao desligada ({temp_c:.1f}°C)",
            )
            self._trigger_action(
                camera_id,
                "heater",
                "on",
                reason=f"Aquecedor ativado preventivamente ({temp_c:.1f}°C < {heater_on_temp:.1f}°C)",
            )
            ...
            self._trigger_action(
                ...
                "heater",
                "off",
                reason=f"Temperatura de conforto reestabelecida. Aquecedor desligado ({temp_c:.1f}°C)",
            )

    def process_ai_vision_anomaly(
        self, camera_id: str, anomaly_type: str, severity: str
    ):
        """
        ...
        if anomaly_type == "thermal_crowding" and severity in ["high", "critical"]:
            logger.warning(
                f"[Automação] IA detectou AGLOMERAÇÃO TÉRMICA extrema em {camera_id}."
            )
            self._trigger_action(
                camera_id,
                "heater",
                "on",
                reason="Aves amontoadas (IA detectou frio extremo)",
            )

        elif anomaly_type == "heat_stress_panting" and severity in ["high", "critical"]:
            logger.warning(
                f"[Automação] IA detectou OFEGANTE/ESTRESSE TÉRMICO em {camera_id}."
            )
            self._trigger_action(
                camera_id,
                "exhaust_fan",
                "on",
                reason="Estresse térmico visual (IA detectou calor)",
            )
            ...
            if humidity < 80.0:
                self._trigger_action(
                    ...
                    "water_pump",
                    "on",
                    reason=f"Ativar nebulização evaporativa (UR: {humidity:.1f}% < 80%)",
                )
            else:
                self._trigger_action(
                    camera_id,
                    "water_pump",
                    "off",
                    reason=f"Bloqueio de nebulização evaporativa por alta umidade (UR: {humidity:.1f}% >= 80%)",
                )

    def process_ai_audio_anomaly(
        self, camera_id: str, cough_idx: float, stress_idx: float
    ):
        """
