...
class BusinessStateMachine:
    ...
    def _determine_state(
        self, intrusion_active: bool, batch_age_day: int, preheat_recommended: bool, hour: int
    ) -> str:
    ...
    def _control_alarm_intruso(
        self, temp: float, fan_on: float, heater_on: float, vent: bool, heat: bool
    ) -> Tuple[bool, bool]:
        """Atua sob alarme de intruso ativo."""
        return (temp >= fan_on, temp <= heater_on)

    def _control_lote_critico(
        self,
        temp: float,
        fan_on: float,
        fan_off: float,
    ...
    def _detect_changes(
        self, vent: bool, vent_prev: bool, heat: bool, heat_prev: bool
    ) -> List[str]:
    ...
    def process_context(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.state == "ALARME_INTRUSO_ATIVO":
            vent, heat = self._control_alarm_intruso(
                temp_atual, fan_on, heater_on, ventilacao_on, aquecedor_on
            )
        elif self.state == "LOTE_DIA_1_AQUECIMENTO_CRITICO":
            vent, heat = self._control_lote_critico(
                temp_atual,
                fan_on,
                fan_off,
                heater_off,
                targets.get("target_temp", 28.0),
                ventilacao_on,
                aquecedor_on,
            )
        elif self.state == "NOITE_POUPANCA_ENERGIA_PREHEAT":
            vent, heat = self._control_noite_preheat(
                temp_atual, fan_off, ventilacao_on, aquecedor_on
            )
        elif self.state == "NOITE_POUPANCA_ENERGIA":
            vent, heat = self._control_noite_poupanca(
                temp_atual, fan_on, fan_off, heater_on, heater_off, ventilacao_on, aquecedor_on
            )
        else:
            vent, heat = self._control_normal(
                temp_atual, fan_on, fan_off, heater_on, heater_off, ventilacao_on, aquecedor_on
            )
        ...
        return {"ventilacao": vent, "aquecedor": heat, "changes": changes, "state": self.state}
