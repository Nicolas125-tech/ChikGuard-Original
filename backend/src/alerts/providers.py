...
class AlertProvider:
    ...
    def _send_to_telegram(self, message):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"[ALERT] Falha no Telegram (Status {response.status_code}): {response.text}")
    ...
    def _send_to_twilio(self, message):
        ...
        if not (self.twilio_account_sid and self.twilio_auth_token and 
                self.twilio_from_number and self.twilio_to_number):
            return
        ...
        try:
            ...
            if response.status_code in {200, 201}:
                ...
                logger.error(f"[ALERT] Falha no Twilio (Status {response.status_code}): {response.text}")
        except Exception as exc:
    ...
    def _send_to_email(self, message):
        ...
        try:
            email_msg = MIMEText(f"O sistema ChikGuard registrou uma ocorrência relevante:\n\n{message}")
