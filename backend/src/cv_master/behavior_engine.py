...
class BehaviorEngine:
    ...
    def update_immobility_and_get_alerts(self, detections):
        ...
        for i in range(len(detections)):
            ...
            if track_id not in self.track_history:
                ...
                if dist > self.immobility_threshold:
                    ...
                    if (
                        time_inactive > self.immobility_time_sec
                        and track_id not in self.dead_or_sick_ids
                    ):
                        self.dead_or_sick_ids.add(track_id)
                        alerts.append(
                            f"ALERTA: Ave ID {track_id} inativa há {int(time_inactive)}s."
                        )
        ...
        ids_to_del = [
            ...
            if (current_time - data["last_seen"]) > 60.0
        ]
