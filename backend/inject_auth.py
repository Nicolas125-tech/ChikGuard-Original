    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            try:
                ...
                if roles and user_role not in roles and "admin" not in user_role:
...
# Insert the middleware after the imports (look for the end of the imports section)
insert_pos = code.find("app = Flask")
if insert_pos > 0:
    code = code[:insert_pos] + auth_middleware_code + "\n" + code[insert_pos:]

# Now apply @require_auth(...) to protected routes
# We'll protect POST routes to devices. Let's find device routes.
# E.g. @app.route("/api/ventilacao", methods=["POST"])
