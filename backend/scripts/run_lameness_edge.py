...
def run_edge_pipeline():
    ...
    try:
        ...
        logger.warning(f"MQTT não disponível em {MQTT_BROKER}. Os eventos serão apenas impressos no log. Erro: {e}")
        client = None
    ...
    while True:
        ...
        for event in lame_events:
            ...
            logger.warning(f"ALERTA: Claudicação Detectada! Ave ID: {event['bird_id']} | Confiança: {event['confidence']*100:.1f}%")
        ...
        cv2.putText(annotated_frame, f"FPS: {fps:.1f}", (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
