...
def test_request_ip_spoof_prevented(app):
    ...
    res = client.get("/", headers={"X-Forwarded-For": "8.8.8.8"}, environ_base={'REMOTE_ADDR': '10.0.0.1'})
    assert res.text == '8.8.8.8'

    # With x_for=1, ProxyFix picks the last IP as the client (8.8.8.8), preventing 1.1.1.1 spoof
    res = client.get("/", headers={"X-Forwarded-For": "1.1.1.1, 8.8.8.8"}, environ_base={'REMOTE_ADDR': '10.0.0.1'})
    assert res.text == '8.8.8.8'
