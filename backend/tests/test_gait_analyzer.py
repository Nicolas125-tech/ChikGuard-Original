...
def test_gait_analyzer_normal_walking():
    ...
    for i in range(15):
        ...
        res = analyzer.update_track(track_id=1, keypoints=kps, timestamp=base_time + timedelta(seconds=i * 0.1))
...
def test_gait_analyzer_claudication():
    ...
    for i in range(15):
        ...
        kps = _generate_keypoints(x, 100, left_foot_y, right_foot_y)
        res = analyzer.update_track(track_id=2, keypoints=kps, timestamp=base_time + timedelta(seconds=i * 0.1))
...
def test_gait_analyzer_lethargy():
    ...
    for i in range(15):
        ...
        res = analyzer.update_track(track_id=3, keypoints=kps, timestamp=base_time + timedelta(seconds=i * 0.1))
