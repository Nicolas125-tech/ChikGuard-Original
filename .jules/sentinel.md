...
## 2026-09-11 - Command Injection in TPMModelLoader **Vulnerability:** Unsanitized tpm_handle_address parameter in TPMModelLoader.__init__ could allow command injection via subprocess.run. **Learning:** Subprocess calls with externally supplied arguments are dangerous if the input is not validated. **Prevention:** Always validate and sanitize variables passed to subprocesses using strict patterns (e.g., regex) or rely on safer subprocess execution methods without shells.
