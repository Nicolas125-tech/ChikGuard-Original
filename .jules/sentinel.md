...
## 2026-06-21 - [Fix Information Exposure in API Error Responses]
**Vulnerability:** Several endpoints in `backend/src/api/auth.py` (e.g., `/api/accounts/users`, `/api/accounts/users/<id>`) were catching broad exceptions and returning the raw Python exception string `str(e)` directly to the client in the 500 error payload (`jsonify({"error": str(e)})`). This is a classic CWE-209 Information Exposure vulnerability that could leak database schemas, internal paths, or library details to attackers.
**Learning:** Returning `str(e)` directly to the client is unsafe. It breaks the "Fail Securely" and "Information Concealment" principles. While debugging details are important, they belong in internal server logs, not HTTP responses.
**Prevention:** Always log the actual exception using Python's standard `logging` module (`logging.error("...", e)`) and return a generic, safe error message (e.g., `"Erro interno no servidor"`) to the client.
