...
## 2025-02-21 - Fix Hardcoded JWT Secret in FastAPI WebSocket
**Vulnerability:** A fallback to a hardcoded string ("dummy_secret_dummy_secret_dummy_secret_for_tests_32bytes") was present when fetching the JWT secret via environment variable for WebSocket authentication. If deployed to production without the environment variable, the app would quietly accept this static secret, potentially allowing attackers to forge tokens.
**Learning:** Always use a single source of truth for secrets, and never use static fallback values in production code. Fail-safe behavior (raising an error or disallowing access if secrets are missing) is much preferred over falling back to weak placeholder strings.
**Prevention:** Avoid writing explicit OR statements with static strings when assigning environment variables (e.g. `SECRET = os.environ.get("SECRET") or "fallback_string"`). Instead, import configured variables from dedicated security/auth modules or raise exceptions immediately.
