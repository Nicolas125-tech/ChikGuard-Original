...
## 2024-05-24 - [Fix SQL Injection Risk in DB Health Check]
**Vulnerability:** Raw SQL execution (`db.session.execute("SELECT 1")`) used for health checks triggers SAST warnings for potential SQL injection, even if the current query is static.
**Learning:** Consistently using SQLAlchemy's `text()` wrapper ensures all SQL statements are explicitly bound and evaluated, avoiding false positives and enforcing a secure coding pattern across the codebase.
**Prevention:** Always wrap raw SQL strings in `text()` when executing them directly via SQLAlchemy sessions to ensure safe evaluation and parameterized execution boundaries.
