...
## 2026-06-20 - [Fix Cross-Site WebSocket Hijacking (CSWSH) and CORS Vulnerability]
**Vulnerability:** The FastAPI application in `backend/main.py` and the SocketIO WebSocket server in `backend/src/api/fastapi_ws.py` were configured with wildcard CORS settings (`allow_origins=["*"]` and `cors_allowed_origins='*'`). This circumvented strict HTTP CORS policies, exposing the application to Cross-Site WebSocket Hijacking (CSWSH) and allowing unauthorized domains to interact with the API and WebSocket streams.
**Learning:** Permissive CORS configurations (like wildcards) during development are often accidentally promoted to production. For WebSockets, this is particularly dangerous as browsers do not enforce Same-Origin Policy for WebSocket connections in the same way they do for standard HTTP requests, relying entirely on the server to validate the `Origin` header.
**Prevention:** Always explicitly restrict `allow_origins` and `cors_allowed_origins` to a defined list of authorized domains (e.g., reusing `ALLOWED_ORIGINS` from `src.security.headers`). Never use a wildcard `*` for CORS or WebSocket origins in production.
