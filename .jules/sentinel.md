...
## 2024-10-09 - Missing dependency-injected permission guards can lead to authentication bypass
**Vulnerability:** Dependency-injected permission guards (like `_guard_critical_action` or `_require_permission`) in Python backend routes (such as in `system_api.py` and `system_api_extra.py`) were not being explicitly checked for presence (e.g. `if not guard:`), allowing endpoints to fail open and bypass authentication if the injection failed.
**Learning:** In Flask/FastAPI applications where security checks are injected via a dependency dictionary, failing to explicitly verify that the injected function exists before calling it can cause the endpoint to execute without authorization checks (fail-open), especially if the code ignores the missing function or hits a silent exception path.
**Prevention:** Always explicitly check for the presence of dependency-injected permission guards (`if not _guard_critical_action: return error`) and fail securely (e.g., return a 500 error) before attempting to call them. This enforces a fail-safe posture.
